...
public class RequiredBlockEntry {
    ...
    public Pair<DetectionResult, String> detect(int count, int size) {
        ...
        if(numericMin && count < min)
            return new Pair<>(DetectionResult.NOT_ENOUGH, String.format("(Current: %d / Total Min: %d)", count, (int) min));
        else if(!numericMin && blockPercent < min)
            return new Pair<>(DetectionResult.NOT_ENOUGH, String.format("(Current: %.2f / Perc. Min: %.2f)", blockPercent, min));

        if(numericMax && count > max)
            return new Pair<>(DetectionResult.TOO_MUCH, String.format("(Current: %d / Total Max: %d)", count, (int) max));
        else if(!numericMax && blockPercent > max)
            return new Pair<>(DetectionResult.TOO_MUCH, String.format("(Current: %.2f / Perc. Max: %.2f)", blockPercent, max));
