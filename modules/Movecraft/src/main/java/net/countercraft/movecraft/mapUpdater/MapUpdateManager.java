...
//    private final Queue<UpdateCommand> updates = new LinkedBlockingQueue<>();
    ...
    //@Deprecated
    //public HashMap<Craft, Integer> blockUpdatesPerCraft = new HashMap<>(128000,1.5f);
    ...
    public void run() {
        ...
        if (!Settings.CompatibilityMode) {
            for (net.minecraft.server.v1_10_R1.Chunk c : chunksToRelight) {
                ChunkUpdater fChunk = FastNMSBlockUtils.getInstance().getChunk(c.world, c.locX, c.locZ, true);
